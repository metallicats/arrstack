  ...
  function rewriteNzbDavLinks(body: any) {
    if (typeof body === 'object' && body && Array.isArray(body.streams)) {
      body.streams.forEach((stream: any) => {
        if (stream.url && stream.url.includes(NZBDAV_URL) && !stream.url.includes('/api/monitor/proxy')) {
          const proto = req.headers['x-forwarded-proto'] || req.protocol;
          const host = req.headers['x-forwarded-host'] || req.get('host');
          const encodedUrl = encodeURIComponent(stream.url);
          stream.url = `${proto}://${host}/api/monitor/proxy?user=${encodeURIComponent(userData)}&url=${encodedUrl}`;
        }
      });
    }
  }
...
export function createMonitorRouter(): Router {
  ...
  router.get('/api/monitor/proxy', (req: Request, res: Response) => {
    ...
    try {
      ...
      const proxyReq = requestFn(options, (proxyRes) => {
        ...
        const logTraffic = () => {
          if (!trafficLogged && bytesSent > 0) {
            trafficLogged = true;
            addTraffic(user, bytesSent);
          }
        };
        ...
        proxyRes.on('end', logTraffic);
        res.on('close', logTraffic);
