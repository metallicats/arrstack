 ...
 *   MONITOR_CONCURRENT_LIMIT  â€” max concurrent streams per user (default: 4)
 *   MONITOR_SESSION_TIMEOUT   â€” ms before idle session expires (default: 300000 = 5min)
 */
...
const NZBDAV_URL = process.env.NZBDAV_URL || 'https://nzbdav.didaskitchen.com';
const NZBDAV_API_KEY = process.env.NZBDAV_API_KEY || 'fe23c947d77c471787a0aed3067bb4b8';
...
async function resolveTitle(contentType: string, contentId: string): Promise<string> {
  ...
  try {
    ...
    // Network error or timeout â€” return raw ID
  }
...
export async function streamMonitorMiddleware(
  ...
  if (existingSession) {
    ...
    // STILL set up interceptors â€” Stremio re-fetches stream list each time
  } else if (currentCount >= CONCURRENT_LIMIT) {
    ...
    logger.warn(
      `Blocked stream for user ${userData.substring(0, 8)}â€¦ â€” ` +
    ...
    res.json({
      streams: [
        {
          name: 'AIOStreams',
          title: `â›” Concurrent stream limit reached (${currentCount}/${CONCURRENT_LIMIT})`,
          description:
    ...
    if (newCount >= CONCURRENT_LIMIT) {
      logger.info(
        `User ${userData.substring(0, 8)}â€¦ at capacity: ${newCount}/${CONCURRENT_LIMIT} streams`
      );
    }
  }

  // --- Response Interception for NzbDAV URL rewriting ---
  // Works at the Node.js level by buffering res.write/res.end calls.
  // This is framework-agnostic and works with Express 4, 5, and any other framework.
  const originalWriteHead = res.writeHead;
  const originalWrite = res.write;
  const originalEnd = res.end;
  const nzbHost = NZBDAV_URL.replace(/^https?:\/\//i, '').toLowerCase();
  ...
  res.writeHead = function (...args: any[]) {
    ...
    if (typeof headers === 'object' && headers !== null) {
      // Headers can be an object or an array of [key, value] pairs
      ct = (headers['content-type'] || headers['Content-Type'] || '').toString().toLowerCase();
    }
    if (ct.includes('application/json')) {
      isJsonResponse = true;
      headArgs = args;  // Defer writeHead until we know the final body
      logger.info(`[Monitor-Intercept] Buffering JSON response for stream request via writeHead`);
      return res;
    }
    return (originalWriteHead as Function).apply(res, args as any);
  } as any;

  res.write = function (chunk: any, ...args: any[]) {
    const ct = (res.getHeader('content-type') || '').toString().toLowerCase();
    if (isJsonResponse || ct.includes('application/json')) {
      isJsonResponse = true;
    ...
    return (originalWrite as Function).apply(res, [chunk, ...args] as any);
  } as any;

  res.end = function (chunk?: any, ...args: any[]) {
    const ct = (res.getHeader('content-type') || '').toString().toLowerCase();
    if (!isJsonResponse && !ct.includes('application/json')) {
      return (originalEnd as Function).apply(res, [chunk, ...args] as any);
    }
    isJsonResponse = true;

    // Collect the final chunk
    if (chunk) {
      if (typeof chunk === 'string') {
        chunks.push(Buffer.from(chunk, 'utf8'));
      } else if (Buffer.isBuffer(chunk)) {
    ...
    try {
      ...
      if (parsed && parsed.streams && Array.isArray(parsed.streams)) {
        let rewriteCount = 0;
        parsed.streams.forEach((s: any) => {
          if (s.url && typeof s.url === 'string') {
            const urlLower = s.url.toLowerCase();
            const isTorbox = urlLower.includes('torbox');
            const isKnownDebrid = urlLower.includes('real-debrid') || urlLower.includes('alldebrid') || urlLower.includes('premiumize');
            
            // Intercept any stream that isn't a known public proxy/debrid service.
            // This ensures NzbDAV streams are caught even if NZBDAV_URL is misconfigured.
            if (!isTorbox && !isKnownDebrid && !urlLower.includes('/monitor/api/proxy') && urlLower.startsWith('http')) {
              const proto = req.headers['x-forwarded-proto'] || req.protocol;
              const host = req.headers['x-forwarded-host'] || req.get('host');
              const encodedUrl = encodeURIComponent(s.url);
              s.url = `${proto}://${host}/monitor/api/proxy?user=${encodeURIComponent(userData)}&url=${encodedUrl}`;
              rewriteCount++;
            }
          }
        });
        if (rewriteCount > 0) {
          ...
          modified = true;
          logger.info(`[Monitor-Intercept] Rewrote ${rewriteCount} NzbDAV URL(s) for user ${userData.substring(0, 12)}...`);
        }
      }
    } catch (e) {
      // Not valid JSON â€” pass through unchanged
      logger.debug(`[Monitor-Intercept] Response was not valid JSON, passing through`);
    }

    const newLength = Buffer.byteLength(body, 'utf8');

    // Fix Content-Length in deferred headers
    if (headArgs) {
      const headerObj = headArgs[headArgs.length - 1];
      if (typeof headerObj === 'object' && headerObj !== null) {
        // Remove any existing content-length (case-insensitive)
        for (const key of Object.keys(headerObj)) {
          if (key.toLowerCase() === 'content-length') {
            headerObj[key] = newLength;
          }
        }
      }
      (originalWriteHead as Function).apply(res, headArgs as any);
    } else {
      // If writeHead wasn't deferred, update Content-Length via res.setHeader if it exists
    ...
    const cb = args.find((a: any) => typeof a === 'function');
    if (cb) {
      return (originalEnd as Function).call(res, body, 'utf8', cb);
    }
    return (originalEnd as Function).call(res, body, 'utf8');
  } as any;
...
async function fetchNzbDavSessions(): Promise<any[]> {
  try {
    if (!NZBDAV_URL || !NZBDAV_API_KEY) return [];
    const url = `${NZBDAV_URL}/api?mode=queue&output=json&apikey=${NZBDAV_API_KEY}`;
    const controller = new AbortController();
    ...
    if (res.ok) {
      ...
      if (data && data.queue && data.queue.slots) {
        return data.queue.slots.map((slot: any) => {
          return {
            uuid: `nzbdav-${slot.nzo_id || Math.random().toString(36).substring(2, 10)}`,
            userConfig: 'Usenet System',
            userConfigShort: 'Usenet System',
            ip: 'nzbdav-server',
            userAgent: 'NzbDAV Server',
            contentType: 'usenet',
            contentId: slot.filename || 'Unknown',
            contentTitle: slot.filename || 'Unknown Download',
            firstSeen: Date.now() - (Math.max(0, parseInt(slot.mb, 10) || 0) * 1000), // Fake start time based on size
            lastSeen: Date.now(),
            requestPath: '/nzbdav/queue',
            status: slot.status === 'Downloading' ? 'downloading' : (slot.status ? slot.status.toLowerCase() : 'queued'),
            service: 'nzbdav',
            duration: 0
          };
        });
    ...
    logger.error(`[Monitor] Failed to fetch NzbDAV queue: ${e.message}`);
  }
  return [];
}
...
const historyDataPath = path.join('/app/data', 'monitor-history.json');
let playbackHistory: HistoryEntry[] = [];
try {
  if (fs.existsSync(historyDataPath)) {
    playbackHistory = JSON.parse(fs.readFileSync(historyDataPath, 'utf-8'));
  }
} catch (e: any) {
  logger.error(`[Monitor] Failed to load history: ${e.message}`);
}

function saveHistoryData() {
  try {
    const dir = path.dirname(historyDataPath);
    if (!fs.existsSync(dir)) {
      fs.mkdirSync(dir, { recursive: true });
    }
    fs.writeFileSync(historyDataPath, JSON.stringify(playbackHistory));
  } catch (e) {
    logger.error(`[Monitor] Failed to save history data:`, e);
  }
}

function addToHistory(session: StreamSession) {
  const existingIndex = playbackHistory.findIndex(entry => 
    entry.userConfig === session.userConfig && entry.contentId === session.contentId
  );
  ...
  const historyEntry: HistoryEntry = {
    ...
    service: (session as any).service || 'nzbdav',
  ...
  playbackHistory.unshift(historyEntry);
...
export interface UserTraffic {
  ...
  lastUpdated: number; // timestamp to reset counters
}
const trafficDataPath = path.join('/app/data', 'monitor-traffic.json');
let userTrafficStats: Record<string, UserTraffic> = {};
try {
  if (fs.existsSync(trafficDataPath)) {
    userTrafficStats = JSON.parse(fs.readFileSync(trafficDataPath, 'utf-8'));
  }
} catch (e: any) {
  logger.error(`[Monitor] Failed to load traffic stats: ${e.message}`);
}
...
function saveTrafficData() {
  ...
  saveTimeout = setTimeout(() => {
    saveTimeout = null;
    try {
      const dir = path.dirname(trafficDataPath);
      if (!fs.existsSync(dir)) {
        fs.mkdirSync(dir, { recursive: true });
      }
      fs.writeFileSync(trafficDataPath, JSON.stringify(userTrafficStats));
    } catch (e) {
      logger.error(`[Monitor] Failed to save traffic data:`, e);
    }
  }, 5000);
}

function resetTrafficIfNewPeriod(stats: UserTraffic) {
  const now = new Date();
  ...
  const getWeek = (d: Date) => {
    const dDate = new Date(Date.UTC(d.getFullYear(), d.getMonth(), d.getDate()));
    ...
    return Math.ceil((((dDate.getTime() - yearStart.getTime()) / 86400000) + 1)/7);
  };
  
  if (getWeek(now) !== getWeek(last) || now.getFullYear() !== last.getFullYear()) {
    stats.weekly = 0;
  }
...
function addTraffic(user: string, bytes: number) {
  if (!userTrafficStats[user]) {
    userTrafficStats[user] = { daily: 0, weekly: 0, monthly: 0, lastUpdated: Date.now() };
  }
  const stats = userTrafficStats[user];
  resetTrafficIfNewPeriod(stats);
  stats.daily += bytes;
  stats.weekly += bytes;
  stats.monthly += bytes;
...
setInterval(saveTrafficData, 60_000);
...
export function createMonitorRouter(): Router {
  ...
  // Proxy Route for Byte Counting
  router.get('/api/proxy', (req: Request, res: Response) => {
    const targetUrlString = req.query.url as string;
    const user = req.query.user as string || 'unknown';
    ...
    try {
      ...
      // Clean up headers from Stremio that shouldn't be forwarded exactly as-is
      ...
      const options = {
        ...
        // Pass auth explicitly if present in URL
        auth: targetUrl.username || targetUrl.password ? `${targetUrl.username}:${targetUrl.password}` : undefined
      };

      const proxyReq = requestFn(targetUrl, options, (proxyRes) => {
        // Rewrite redirects to keep Stremio on the proxy
        if (proxyRes.statusCode && proxyRes.statusCode >= 300 && proxyRes.statusCode < 400 && proxyRes.headers.location) {
          try {
            const redirectUrl = new URL(proxyRes.headers.location, targetUrlString).toString();
            const proto = req.headers['x-forwarded-proto'] || req.protocol;
            const host = req.headers['x-forwarded-host'] || req.get('host');
            const encodedUrl = encodeURIComponent(redirectUrl);
            proxyRes.headers.location = `${proto}://${host}/monitor/api/proxy?user=${encodeURIComponent(user)}&url=${encodedUrl}`;
          } catch (e) {
            // Ignore parse errors
          }
        }
        ...
        proxyRes.on('data', (chunk) => {
          // Log bytes incrementally in real-time as they stream
          addTraffic(user, chunk.length);
        });

        // Pipe directly to response to automatically handle backpressure
        proxyRes.pipe(res);
      });

      // For GET/HEAD requests, the request body is empty and might have already emitted 'end'.
      // If we pipe an already-ended stream, proxyReq never closes, causing Stremio to hang forever.
      ...
      proxyReq.on('error', (err: any) => {
        logger.error(`[Monitor] Proxy error: ${err.message}`);
      ...
      logger.error(`[Monitor] Invalid proxy URL: ${targetUrlString}`);
  ...
  router.get('/api/traffic-stats', (_req: Request, res: Response) => {
    // Refresh limits before sending
    for (const user in userTrafficStats) {
      resetTrafficIfNewPeriod(userTrafficStats[user]);
    }
    res.json(userTrafficStats);
  });
  ...
  router.get('/api/sessions', async (_req: Request, res: Response) => {
    ...
    const mappedSessions = allSessions.map((s) => {
        ...
        return {
          ...
          userConfigShort: s.userConfig.substring(0, 12) + 'â€¦',
          duration: Date.now() - s.firstSeen,
        };
    });

    const nzbdavSessions = await fetchNzbDavSessions();
    const combinedSessions = [...mappedSessions, ...nzbdavSessions];

    res.json({
      sessions: combinedSessions,
      concurrentLimit: CONCURRENT_LIMIT,
      activeCount: combinedSessions.length,
      uniqueUsers,
      uniqueIPs,
    });
  });
  ...
  let prevNetStats: Record<string, { timestamp: number; rxBytes: number; txBytes: number }> = {};
  let networkSource = 'none';

  function readNetworkStats(): Record<string, { rxBytes: number; txBytes: number }> | null {
    const procPaths = ['/host/proc/1/net/dev', '/host/proc/net/dev', '/proc/net/dev'];
    for (const procPath of procPaths) {
      try {
        const data = fs.readFileSync(procPath, 'utf-8');
        const lines = data.split('\n').slice(2);
        
        let stats: Record<string, { rxBytes: number; txBytes: number }> = {};
        let hasData = false;
        ...
        for (const line of lines) {
          ...
          if (/^lo/i.test(iface)) continue; // Always ignore loopback
  ...
  router.get('/api/network', (_req: Request, res: Response) => {
    ...
    let maxRxRate = 0;
    let maxTxRate = 0;
    let maxCombinedRate = -1;

    for (const iface in current) {
       const currStats = current[iface];
       const prevStats = prevNetStats[iface];
       
       let rxRate = 0;
       let txRate = 0;

       if (prevStats) {
         const elapsed = (now - prevStats.timestamp) / 1000;
         if (elapsed > 0) {
           rxRate = Math.max(0, (currStats.rxBytes - prevStats.rxBytes) / elapsed);
           txRate = Math.max(0, (currStats.txBytes - prevStats.txBytes) / elapsed);
         }
       }
       
       const combined = rxRate + txRate;
       if (combined > maxCombinedRate) {
           maxCombinedRate = combined;
           maxRxRate = rxRate;
           maxTxRate = txRate;
       }
       
       prevNetStats[iface] = { timestamp: now, rxBytes: currStats.rxBytes, txBytes: currStats.txBytes };
    }

    res.json({
      rxBytesPerSec: Math.round(maxRxRate),
      txBytesPerSec: Math.round(maxTxRate),
...
// ============================================
// AIOStreams Monitor â€” Dashboard Application
// ============================================
...
(function() {
    ...
    // API base path â€” when served from AIOStreams, this is relative.
    ...
    const DEBRID_SERVICES = {
        nzbdav:  { name: 'NzbDAV', color: '#10b981' },
        torbox:  { name: 'TorBox', color: '#8b5cf6' },
    ...
    function cacheDom() {
        ...
        dom.trafficBody = document.getElementById('trafficBody');
        dom.historyBody = document.getElementById('historyBody');
        dom.trafficEmptyState = document.getElementById('trafficEmptyState');
        ...
        try {
            ...
            state.sessions = data.sessions.map(s => {
                ...
                return {
                    ...
                    user: actualUserId.substring(0, 12) + 'â€¦',
    ...
    function triggerViolation(session) {
        ...
        addAlert('critical', 'Concurrent Stream Violation!',
            `${state.sessions.length} active streams exceed limit of ${state.concurrentLimit}. Session: ${truncateUUID(session.id)} â€” ${session.user} (${session.ip})`,
            Date.now());

        showToast('critical', 'ðŸš¨ Stream Violation Detected',
            `${state.sessions.length}/${state.concurrentLimit} concurrent streams â€” limit exceeded!`);
        ...
        if (state.alerts.length === 0) {
            dom.alertsList.innerHTML = `
                <div class="empty-state">
                    ...
                    <span>No alerts â€” all clear!</span>
                </div>`;
            return;
        }
    ...
    function updateKPIs() {
        ...
        if (uniqueIPs > 3) {
            ipEl.innerHTML = '<span class="ip-danger">âš  Multiple IPs detected</span>';
        } else if (uniqueIPs > 2) {
    ...
    async function updateNetworkStats() {
        if (!isLiveMode) {
            ...
            if (dom.networkDetails) {
                dom.networkDetails.innerHTML = `<span class="network-stats">&darr; ${formatBytesRate(fakeRx)} &nbsp; &uarr; ${formatBytesRate(fakeTx)}</span>`;
            }
            return;
        }
        try {
            ...
            if (dom.networkDetails) {
                ...
                dom.networkDetails.innerHTML = `<span class="network-stats">&darr; ${formatBytesRate(data.rxBytesPerSec)} &nbsp; &uarr; ${formatBytesRate(data.txBytesPerSec)}${warning}</span>`;
            }
        } catch (e) {
            if (dom.networkBandwidth) {
    ...
    async function fetchTrafficStats() {
        ...
        try {
            const data = await apiFetch('/traffic-stats');
            renderTraffic(data);
        } catch (e) {
            console.warn('Failed to fetch traffic stats:', e);
        }
    }
    ...
    function renderTraffic(data) {
        ...
        const rows = Object.entries(data).map(([user, stats]) => {
            return `<tr>
                <td>
                    <div class="user-cell">
                        <span class="user-badge">${user.substring(0, 2).toUpperCase()}</span>
                        <span>${user.substring(0, 12)}â€¦</span>
                    </div>
                </td>
                <td class="font-mono">${formatBytesTotal(stats.daily)}</td>
                <td class="font-mono">${formatBytesTotal(stats.weekly)}</td>
                <td class="font-mono">${formatBytesTotal(stats.monthly)}</td>
            </tr>`;
        });

        if (rows.length === 0) {
            dom.trafficBody.innerHTML = '<tr><td colspan="4" style="text-align: center; padding: 2rem; color: var(--text-tertiary);">No traffic recorded yet</td></tr>';
            if (dom.trafficEmptyState) dom.trafficEmptyState.classList.remove('hidden');
        } else {
            dom.trafficBody.innerHTML = rows.join('');
            if (dom.trafficEmptyState) dom.trafficEmptyState.classList.add('hidden');
        }
    }
    ...
    function renderHistory(data) {
        ...
        const rows = data.map((entry) => {
            const serviceData = DEBRID_SERVICES[entry.service] || DEBRID_SERVICES.unknown;
            const date = new Date(entry.firstSeen).toLocaleString();
            return `<tr>
                <td style="white-space: nowrap;">${date}</td>
                <td>
                    <div class="user-cell">
                        <span class="user-badge">${entry.userConfig.substring(0, 2).toUpperCase()}</span>
                        <span>${entry.userConfig.substring(0, 12)}â€¦</span>
                    </div>
                </td>
                <td><span class="service-tag" style="background: ${serviceData.color}15; color: ${serviceData.color}; border-color: ${serviceData.color}30">${serviceData.name}</span></td>
                <td>${entry.contentTitle}</td>
                <td class="font-mono text-xs" style="color: var(--text-tertiary)">${entry.uuid.substring(0, 8)}â€¦</td>
            </tr>`;
        });

        if (rows.length === 0) {
            dom.historyBody.innerHTML = '<tr><td colspan="5" style="text-align: center; padding: 2rem; color: var(--text-tertiary);">No playback history yet</td></tr>';
        } else {
    ...
    function drawDonut() {
        ...
        entries.forEach(([service, count]) => {
            ...
            const serviceData = DEBRID_SERVICES[service] || DEBRID_SERVICES.unknown;
        ...
        dom.donutLegend.innerHTML = entries.map(([service, count]) => {
            const sd = DEBRID_SERVICES[service] || DEBRID_SERVICES.unknown;
    ...
    // ============================================
    // Charts â€” Violation History Bar Chart
    // ============================================
    ...
    function startPolling() {
            ...
            try {
                ...
                if (violationData.violations) {
                    state.violations = violationData.violations.map(v => ({
                        ...
                        sessionId: v.uuid,
                        user: v.userConfig?.substring(0, 12) + 'â€¦',
                        ip: v.ip,
                        count: v.activeCount,
                        limit: v.limit,
                    }));
                    ...
                    if (state.violations.length > prevViolationCount) {
                        ...
                        if (latest && latest.blocked) {
                            addAlert('critical', 'ðŸš« Stream Blocked!',
                                `User ${latest.userConfig?.substring(0, 8)}â€¦ blocked â€” ${latest.activeCount}/${latest.limit} streams`,
                                latest.timestamp);
                            showToast('critical', 'ðŸš« Stream Blocked',
                                `Concurrent limit exceeded â€” stream request was rejected`);
                            dom.kpiViolations.classList.add('violation-active');
                            setTimeout(() => dom.kpiViolations.classList.remove('violation-active'), 5000);
                        }
                    }
        ...
        // Poll traffic stats every 10 seconds
        fetchTrafficStats();
        setInterval(() => {
            fetchTrafficStats();
        }, 10000);
    ...
    async function init() {
        ...
        if (live) {
            ...
            addAlert('info', 'Dashboard Online', 'Connected to AIOStreams â€” monitoring live concurrent streams', Date.now());
            ...
            setTimeout(() => {
                showToast('success', 'Live Mode', 'Connected to AIOStreams API â€” real-time monitoring active');
            }, 500);
            ...
            addAlert('info', 'Demo Mode', 'API not available â€” showing simulated data', Date.now());
            ...
            setTimeout(() => {
                showToast('info', 'Demo Mode', 'API not available â€” using simulated data');
            }, 1500);
...
/* ============================================
   AIOStreams Monitor â€” Design System
   ============================================ */
...
@media (max-width: 1200px) {
    ...
    .card-traffic { grid-column: 1 / -1; grid-row: auto; }
...
<head>
    ...
    <title>AIOStreams Monitor â€” Concurrent Stream Violation Dashboard</title>
    ...
    <div class="app-container">
        ...
        <main class="main-content">
            ...
            <section class="kpi-row" id="kpiRow">
                ...
                <div class="kpi-card" id="kpiNetwork">
                    ...
                    <div class="kpi-data">
                        ...
                        <span class="kpi-label">Server Traffic</span>
                    </div>
                    <div class="kpi-sub" id="networkDetails">
                        <span class="network-stats">â†“ 0 B/s &nbsp; â†‘ 0 B/s</span>
                    </div>
                </div>
            </section>
            ...
            <section class="dashboard-grid">
                    ...
                    <div class="card-header">
                        ...
                        <div class="card-actions">
                            <div class="search-wrap">
                                ...
                                <input type="text" placeholder="Search sessionsâ€¦" id="sessionSearch" class="search-input">
                            </div>
                        </div>
                    </div>
                ...
                <!-- User Traffic Table -->
                <div class="card card-traffic" id="trafficCard">
                    <div class="card-header">
                        <h2 class="card-title">User Traffic (NzbDAV)</h2>
                    </div>
                    <div class="table-scroll" id="trafficTableScroll">
                        <table class="sessions-table" id="trafficTable">
                            <thead>
                                <tr>
                                    <th>User Config</th>
                                    <th>Daily</th>
                ...
                <div class="card card-history" id="historyCard">
                    <div class="card-header">
                        <h2 class="card-title">Playback History</h2>
                    </div>
                    <div class="table-scroll" id="historyTableScroll">
                        <table class="sessions-table" id="historyTable">
                            <thead>
                                <tr>
                                    <th>Date / Time</th>
                                    <th>User / Device</th>
...
if [ -f "docker-compose.yml" ]; then
    ...
    # Remove old specific dev mount if it exists to prevent Docker mount conflicts
    sed -i '\#- /proc/net/dev:/host/proc/net/dev:ro#d' docker-compose.yml
    
    # Automatically inject host network stats volume if missing
    if ! grep -q "/proc:/host/proc:ro" docker-compose.yml; then
        sed -i 's|- nzbdav_shared:/data|- nzbdav_shared:/data\n      - /proc:/host/proc:ro|g' docker-compose.yml
        sed -i 's|- aiostreams_data:/app/data|- aiostreams_data:/app/data\n      - /proc:/host/proc:ro|g' docker-compose.yml
        # Remove duplicate if both triggered
        awk '!seen[]++' docker-compose.yml > docker-compose.tmp && mv docker-compose.tmp docker-compose.yml
        echo "Injected /proc volume mount into docker-compose.yml."
    fi
