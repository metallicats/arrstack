    ...
    for (const procPath of procPaths) {
      try {
        ...
        for (const line of lines) {
          ...
          // Exclude virtual/local loopback interfaces to sum up total host network utilization
          const isVirtualOrLoopback = /^(lo|docker|br-|veth|cni|cali|flannel|tun|tap|virbr|wg|tailscale)/i.test(iface);
    ...
    async function updateNetworkStats() {
        if (!isLiveMode) {
            ...
            if (dom.networkDetails) {
                dom.networkDetails.innerHTML = `<span class="network-stats">â†“ ${formatBytesRate(fakeRx)} &nbsp; â†‘ ${formatBytesRate(fakeTx)}</span>`;
            }
            return;
        }
        try {
            ...
            if (dom.networkDetails) {
                ...
                dom.networkDetails.innerHTML = `<span class="network-stats">â†“ ${formatBytesRate(data.rxBytesPerSec)} &nbsp; â†‘ ${formatBytesRate(data.txBytesPerSec)}${warning}</span>`;
            }
        } catch (e) {
            if (dom.networkBandwidth) {
