...
export async function streamMonitorMiddleware(
  ...
  // --- Proxy Intercept for NzbDAV streams ---
  const originalJson = res.json;
  const originalSend = res.send;
  const originalEnd = res.end;

  function rewriteNzbDavLinks(body: any) {
    if (typeof body === 'object' && body && Array.isArray(body.streams)) {
      body.streams.forEach((stream: any) => {
        if (stream.url && typeof stream.url === 'string') {
          ...
          if ((urlStr.includes('nzbdav') || urlStr.includes(nzbHost)) && !urlStr.includes('/monitor/api/proxy')) {
            const proto = req.headers['x-forwarded-proto'] || req.protocol;
            const host = req.headers['x-forwarded-host'] || req.get('host');
            const encodedUrl = encodeURIComponent(stream.url);
            stream.url = `${proto}://${host}/monitor/api/proxy?user=${encodeURIComponent(userData)}&url=${encodedUrl}`;
          }
        }
      });
    }
  ...
  res.end = function(chunk?: any, encodingOrCb?: any, cb?: any) {
    if (chunk && typeof chunk === 'string') {
      ...
      try {
        ...
        if (parsed.streams) {
          rewriteNzbDavLinks(parsed);
          chunk = Buffer.from(JSON.stringify(parsed), 'utf8');
        }
      } catch (e) {}
    }
