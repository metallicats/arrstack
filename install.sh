...
function saveTrafficStats() {
  try {
    fs.writeFileSync(trafficDataPath, JSON.stringify(userTrafficStats, null, 2));
  } catch (e: any) {
    logger.error(`[Monitor] Failed to save traffic stats: ${e.message}`);
  }
}
...
(function() {
    ...
    function renderTraffic(data) {
        ...
        const rows = Object.entries(data).map(([user, stats]) => {
            return `
                <tr>
                    <td><span class="session-uuid" title="${user}">${user.substring(0, 16)}${user.length > 16 ? '...' : ''}</span></td>
                    <td><span class="session-duration">${formatBytesTotal(stats.daily)}</span></td>
                    <td><span class="session-duration">${formatBytesTotal(stats.weekly)}</span></td>
                    <td><span class="session-duration">${formatBytesTotal(stats.monthly)}</span></td>
                </tr>
            `;
        }).join('');
